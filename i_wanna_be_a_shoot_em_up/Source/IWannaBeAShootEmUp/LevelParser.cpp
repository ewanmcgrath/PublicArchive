...
Level* LevelParser::parseLevel(const char *levelFile)
{
    ...
    for(TiXmlElement* e = pRoot->FirstChildElement(); e != NULL; e = e->NextSiblingElement())
    {
        if(e->Value() == std::string("objectgroup") || e->Value() == std::string("layer"))
        {
            ...
            {
                parseTileLayer(e, pLevel->getLayers(), pLevel->getTilesets(), pLevel->getCollisionLayers()); // This also parses the Tile Layers, identified if they have a "data" group, 
            }
        }
    }
...
void LevelParser::parseTilesets(TiXmlElement* pTilesetRoot, std::vector<Tileset>* pTilesets)
{
    ...
    pTilesetRoot->Attribute("tilecount", &tileset.spacing);
    pTilesetRoot->Attribute("columns", &tileset.margin);
    tileset.name = pTilesetRoot->Attribute("name");

    tileset.numColumns =  tileset.spacing;
...
void LevelParser::parseObjectLayer(TiXmlElement* pObjectElement, std::vector<Layer*> *pLayers, Level* pLevel)
{
    ...
    for(TiXmlElement* e = pObjectElement->FirstChildElement(); e != NULL; e = e->NextSiblingElement())
    {
        if(e->Value() == std::string("object"))
        {
			...
			// give the gameobject a set of the collision layers. TODO Improve this design
			...
			else if (type == "Assassin" || type == "Newborn" || type == "Uno" || type == "Boss") // Improve this to count all real enemies
			{
				TheGame::Instance()->setLevelEnemyCount(TheGame::Instance()->getLevelEnemyCount() + 1);
			}
	...
	std::cout << "Object Layer Created, Amount of Layers" << pLayers->size() << std::endl;
}
...
void LevelParser::parseTileLayer(TiXmlElement* pTileElement, std::vector<Layer*> *pLayers, const std::vector<Tileset>* pTilesets, std::vector<TileLayer*> *pCollisionLayers)
{
    ...
    if(collidable)
    {
		...
		std::cout << "Collision Layer Size: " << pCollisionLayers->size() << "\n";
    }
    pLayers->push_back(pTileLayer);

	std::cout << "Tile Layer Created, Amount of Layers" << pLayers->size() << std::endl;
}
